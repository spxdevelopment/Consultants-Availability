...
(() => {
  ...
  function renderWeeklyGrid(data) {
    ...
    data.consultants.forEach((consultant) => {
      ...
      nameTd.addEventListener("click", () => {
        // If a details row already follows this row, remove it
        const next = tr.nextSibling;
        if (next && next.classList && next.classList.contains("detail-row")) {
          tr.parentNode.removeChild(next);
          return;
        }
        // Compute aggregated allocations across the selected week periods
        const aggAlloc = {};
        let countWeeks = weekPeriods.length;
        data.projects.forEach((proj) => {
          aggAlloc[proj] = 0;
        });
        weekPeriods.forEach((period) => {
        ...
        // Span across all columns (consultant + weeks)
        detailTd.colSpan = weekPeriods.length + 1;
        // Create details container with table listing projects and average allocation
        ...
        const detHeadRow = document.createElement("tr");
        ["Project", "Average Allocation (%)"].forEach((txt) => {
          const th = document.createElement("th");
          th.textContent = txt;
          detHeadRow.appendChild(th);
        });
        detHead.appendChild(detHeadRow);
        ...
        data.projects.forEach((proj) => {
          ...
          const avg = countWeeks > 0 ? aggAlloc[proj] / countWeeks : 0;
          tdVal.textContent = avg.toFixed(2);
          trp.appendChild(tdProj);
          trp.appendChild(tdVal);
          detBody.appendChild(trp);
        });
  ...
  function renderMonthlyGrid(data) {
    ...
    data.consultants.forEach((consultant) => {
      ...
      nameTd.addEventListener("click", () => {
        // Toggle details row for monthly view similar to weekly
        const next = tr.nextSibling;
        if (next && next.classList && next.classList.contains("detail-row")) {
          tr.parentNode.removeChild(next);
          return;
        }
        // Compute aggregated allocations across monthPeriods
        const aggAlloc = {};
        const countMonths = monthPeriods.length;
        data.projects.forEach((proj) => {
          aggAlloc[proj] = 0;
        });
        monthPeriods.forEach((period) => {
        ...
        detHead.appendChild(headRow);
        detTable.appendChild(detHead);
        const detBody = document.createElement("tbody");
        data.projects.forEach((proj) => {
          const r = document.createElement("tr");
          const tdP = document.createElement("td");
          tdP.textContent = proj;
          const tdV = document.createElement("td");
          const avg = countMonths > 0 ? aggAlloc[proj] / countMonths : 0;
          tdV.textContent = avg.toFixed(2);
          r.appendChild(tdP);
          r.appendChild(tdV);
          detBody.appendChild(r);
        });
        detTable.appendChild(detBody);
        detailsDiv.appendChild(detTable);
        detailTd.appendChild(detailsDiv);
