  ...
  function renderWeeklyGrid(data) {
    ...
    data.consultants.forEach((consultant) => {
      ...
      weekPeriods.forEach((period) => {
        ...
        // Collapsed view: single bar showing total allocation in primary colour
        const collapsedDiv = document.createElement("div");
        collapsedDiv.className = "cell-collapsed";
        const collapsedBar = document.createElement("div");
        collapsedBar.className = "cell-bar";
        collapsedBar.style.position = "relative";
        const collapsedSeg = document.createElement("div");
        collapsedSeg.className = "cell-segment";
        const collapsedWidth = total > 0 ? Math.min(100, total) : 0;
        collapsedSeg.style.width = collapsedWidth + "%";
        collapsedSeg.style.backgroundColor = summaryColor;
        collapsedBar.appendChild(collapsedSeg);
        // Fill remaining if total < 100
        if (collapsedWidth < 100) {
          const emptySeg = document.createElement("div");
          emptySeg.className = "cell-segment";
          emptySeg.style.width = (100 - collapsedWidth) + "%";
          emptySeg.style.backgroundColor = "#f5f5f5";
          collapsedBar.appendChild(emptySeg);
        }
        // Overlay total percentage text centered
        ...
        overlay.style.color = collapsedWidth > 50 ? "#ffffff" : "#000000";
        collapsedBar.appendChild(overlay);
        collapsedDiv.appendChild(collapsedBar);
        // Expanded view: segments per project with percentages
        const expandedDiv = document.createElement("div");
        expandedDiv.className = "cell-expanded";
        if (total > 0) {
          const expBar = document.createElement("div");
          expBar.className = "cell-bar";
          ...
          data.projects.forEach((proj) => {
            ...
            if (value > 0) {
              ...
              expBar.appendChild(seg);
            }
          });
          expandedDiv.appendChild(expBar);
        } else {
          ...
          expandedDiv.appendChild(emptyBar);
        }
        td.appendChild(collapsedDiv);
        td.appendChild(expandedDiv);
        tr.appendChild(td);
      });
      // Toggle expansion on clicking consultant name
      nameTd.addEventListener("click", () => {
        const isExpanded = tr.classList.toggle("expanded");
        // The CSS rules will handle showing/hiding collapsed/expanded cells
      });
  ...
  function renderMonthlyGrid(data) {
    ...
    data.consultants.forEach((consultant) => {
      ...
      monthPeriods.forEach((period) => {
        ...
        const seg = document.createElement("div");
        seg.className = "cell-segment";
        const width = total > 0 ? Math.min(100, total) : 0;
        seg.style.width = width + "%";
        seg.style.backgroundColor = summaryColor;
        collapsedBar.appendChild(seg);
        if (width < 100) {
          const emptySeg = document.createElement("div");
          emptySeg.className = "cell-segment";
          emptySeg.style.width = (100 - width) + "%";
          emptySeg.style.backgroundColor = "#f5f5f5";
          collapsedBar.appendChild(emptySeg);
        }
        const overlay = document.createElement("span");
        ...
        overlay.style.fontSize = "0.6rem";
        overlay.style.fontWeight = "bold";
        overlay.style.color = width > 50 ? "#ffffff" : "#000000";
        collapsedBar.appendChild(overlay);
        collapsedDiv.appendChild(collapsedBar);
        // Expanded view
        const expandedDiv = document.createElement("div");
        ...
        if (total > 0) {
          ...
          data.projects.forEach((proj) => {
            ...
            if (val > 0) {
              ...
              segm.className = "cell-segment";
              segm.style.width = (val * factor) + "%";
              segm.style.backgroundColor = projectColors[proj];
              segm.textContent = val + "%";
              segm.style.fontSize = "0.6rem";
              segm.style.color = getContrastColor(projectColors[proj]);
              segm.style.display = "flex";
              segm.style.alignItems = "center";
              segm.style.justifyContent = "center";
              expBar.appendChild(segm);
            }
          });
          expandedDiv.appendChild(expBar);
        } else {
