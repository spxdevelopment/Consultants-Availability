...
(() => {
  ...
  function renderSummary(data) {
    ...
    data.consultants.forEach((consultant) => {
      ...
      // Allocation bar cell (collapsed view)
      const bar = document.createElement("div");
      bar.className = "allocation-bar";
      // Determine scaling factor if total > 100
      const factor = total > 0 ? Math.min(1, 100 / total) : 1;
      data.projects.forEach((proj) => {
        const value = avgAlloc[proj];
        ...
        if (value > 0) {
          ...
          // Show total percentage only on a combined bar when not expanded; we
          // don't include individual labels here to keep the bar clean.  The
          // percentage for each project is displayed in the details table.
        } else {
          seg.style.backgroundColor = "transparent";
        }
        bar.appendChild(seg);
      });
      // Append empty segment for unused capacity
      if (total < 100) {
      ...
      overlay.style.color = total > 50 ? "#ffffff" : "#000000";
