...
(() => {
  ...
  function renderWeeklyGrid(data) {
    ...
    data.consultants.forEach((consultant) => {
      ...
      nameTd.addEventListener("click", () => {
        // Toggle expansion: if next rows are detail rows, remove them
        let next = tr.nextSibling;
        if (next && next.classList && next.classList.contains("detail-row")) {
          while (next && next.classList && next.classList.contains("detail-row")) {
            const toRemove = next;
        ...
        // Determine projects this consultant works on for at least one selected week
        ...
        // For each project, insert a new row aligned with the main table columns
        // Insert projects in reverse order to preserve original ordering in display
        projList.slice().reverse().forEach((proj) => {
          const detailRow = document.createElement("tr");
          detailRow.className = "detail-row";
          // Project name cell with indentation arrow
          const nameCell = document.createElement("td");
          nameCell.textContent = proj;
          nameCell.style.paddingLeft = "20px";
          detailRow.appendChild(nameCell);
          // Create a cell for each week period
          weekPeriods.forEach((period) => {
            const cell = document.createElement("td");
            const allocs = (data.periods.week[period] && data.periods.week[period][consultant]) || {};
            const val = allocs[proj] || 0;
            if (val > 0) {
              const color = projectColors[proj] || summaryColor;
  ...
  function renderMonthlyGrid(data) {
    ...
    data.consultants.forEach((consultant) => {
      ...
      nameTd.addEventListener("click", () => {
        // Toggle: remove consecutive detail rows if present
        let next = tr.nextSibling;
        if (next && next.classList && next.classList.contains("detail-row")) {
          while (next && next.classList && next.classList.contains("detail-row")) {
            const toRemove = next;
        ...
        const projList = data.projects.filter((p) => activeProjects.has(p));
        // For each project, create a detail row aligned with month columns
        // Insert projects in reverse order to preserve original ordering in display
        projList.slice().reverse().forEach((proj) => {
          const detailRow = document.createElement("tr");
          detailRow.className = "detail-row";
