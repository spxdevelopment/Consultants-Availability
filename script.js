...
(() => {
  ...
  function renderSummary(data) {
    // Only render grid if a date range is selected
    if (!selectedStartDate || !selectedEndDate) return;
    // Update period info banner to reflect selected range
    updatePeriodInfo();
    // Delegate to grid renderer based on view mode
  ...
  function renderWeeklyGrid(data) {
    ...
    // Clear container
    container.innerHTML = "";
    // Only render if start and end dates are set
    if (!selectedStartDate || !selectedEndDate) return;
    // Collect all week keys and filter by date range
    const allWeeks = Object.keys(data.periods.week).sort();
    const weekPeriods = allWeeks.filter((period) => {
      const { start, end } = getWeekStartEnd(period);
      return (
    ...
    // Header for consultant names
    const blankTh = document.createElement("th");
    blankTh.textContent = "Consultant";
    headRow.appendChild(blankTh);
    // Headers for each week showing the Friday (end-of-week) date
    weekPeriods.forEach((period) => {
    ...
    data.consultants.forEach((consultant) => {
      ...
      // Add name cell and attach click handler to toggle expanded state
      ...
      weekPeriods.forEach((period) => {
        ...
        // Build a single bar for the cell representing the total allocation.
        const cellBar = document.createElement("div");
        cellBar.className = "cell-bar";
        cellBar.style.position = "relative";
        // Filled segment showing total percentage in brand colour
        const filledSeg = document.createElement("div");
        filledSeg.className = "cell-segment";
        const filledWidth = total > 0 ? Math.min(100, total) : 0;
        filledSeg.style.width = filledWidth + "%";
        filledSeg.style.backgroundColor = summaryColor;
        cellBar.appendChild(filledSeg);
        // Remaining empty segment
        if (filledWidth < 100) {
          const emptySeg = document.createElement("div");
          emptySeg.className = "cell-segment";
          emptySeg.style.width = (100 - filledWidth) + "%";
          emptySeg.style.backgroundColor = "#f5f5f5";
          cellBar.appendChild(emptySeg);
        }
        // Overlay total percentage text
        const overlay = document.createElement("span");
        overlay.textContent = total > 0 ? Math.round(total) + "%" : "0%";
        overlay.style.position = "absolute";
        overlay.style.left = "50%";
        overlay.style.top = "50%";
        overlay.style.transform = "translate(-50%, -50%)";
        overlay.style.fontSize = "0.6rem";
        overlay.style.fontWeight = "bold";
        overlay.style.color = filledWidth > 50 ? "#ffffff" : "#000000";
        cellBar.appendChild(overlay);
        td.appendChild(cellBar);
        tr.appendChild(td);
      });
      // When clicking the consultant name, toggle a details row showing
      // allocations per project across the selected weeks.
      nameTd.addEventListener("click", () => {
        // Toggle details row: remove if exists
        const next = tr.nextSibling;
        if (next && next.classList && next.classList.contains("detail-row")) {
          tr.parentNode.removeChild(next);
          return;
        }
        // Determine list of projects the consultant works on in any of the selected weeks
        const activeProjects = new Set();
        weekPeriods.forEach((period) => {
          const allocs = (data.periods.week[period] && data.periods.week[period][consultant]) || {};
          Object.keys(allocs).forEach((proj) => {
            if (allocs[proj] > 0) activeProjects.add(proj);
          });
        });
        // Convert Set to array and maintain original order from data.projects
        const projList = data.projects.filter((p) => activeProjects.has(p));
        // Build details row with a table showing allocations per week and project
        const detailTr = document.createElement("tr");
        detailTr.className = "detail-row";
        const detailTd = document.createElement("td");
        detailTd.colSpan = weekPeriods.length + 1;
        // Create details container
        const detailsDiv = document.createElement("div");
        detailsDiv.className = "details";
        const detTable = document.createElement("table");
        detTable.className = "details-table";
        // Build header: first cell blank (Project), then one for each week
        ...
        detHead.appendChild(headerRow);
        detTable.appendChild(detHead);
        // Build body: one row per active project
        const detBody = document.createElement("tbody");
        projList.forEach((proj) => {
          const row = document.createElement("tr");
          const projTd = document.createElement("td");
          projTd.textContent = proj;
          row.appendChild(projTd);
          weekPeriods.forEach((period) => {
            const cell = document.createElement("td");
            const allocs = (data.periods.week[period] && data.periods.week[period][consultant]) || {};
            const val = allocs[proj] || 0;
            if (val > 0) {
              cell.style.backgroundColor = projectColors[proj] || summaryColor;
              cell.style.color = getContrastColor(projectColors[proj] || summaryColor);
        ...
        detailsDiv.appendChild(detTable);
        detailTd.appendChild(detailsDiv);
  ...
  /**
   * Render a monthly grid showing allocations per consultant across months
   * within the selected date range. Each column corresponds to a month and
   * cells contain either a collapsed total bar (green) or an expanded set
   * of coloured segments per project. Clicking the consultant name toggles
   * expansion across the row.  This mirrors the behaviour of the weekly
   * grid but aggregates by month instead of week.
   *
   * @param {Object} data - The dataset loaded from storage.
   */
  function renderMonthlyGrid(data) {
    ...
    container.innerHTML = "";
    if (!selectedStartDate || !selectedEndDate) return;
    // Build list of month periods within range
    const allMonths = Object.keys(data.periods.month).sort();
    const monthPeriods = allMonths.filter((period) => {
    ...
    const table = document.createElement("table");
    table.className = "weekly-grid-table";
    // Use the same class for styling; monthly grid cells use the same CSS
    const thead = document.createElement("thead");
    const headRow = document.createElement("tr");
    const blankTh = document.createElement("th");
    blankTh.textContent = "Consultant";
    ...
    monthPeriods.forEach((period) => {
      const th = document.createElement("th");
      // Display as "MMM YYYY" for month header
      const { start } = getMonthStartEnd(period);
      th.textContent = start.toLocaleDateString(undefined, {
        month: "short",
        year: "numeric",
      });
      headRow.appendChild(th);
    ...
    data.consultants.forEach((consultant) => {
      ...
      monthPeriods.forEach((period) => {
        const td = document.createElement("td");
        const allocations = (data.periods.month[period] && data.periods.month[period][consultant]) || {};
        const total = Object.values(allocations).reduce((sum, v) => sum + v, 0);
        // Build a single bar showing total allocation in brand colour
        const cellBar = document.createElement("div");
        cellBar.className = "cell-bar";
        cellBar.style.position = "relative";
        const filled = document.createElement("div");
        filled.className = "cell-segment";
        const widthVal = total > 0 ? Math.min(100, total) : 0;
        filled.style.width = widthVal + "%";
        filled.style.backgroundColor = summaryColor;
        cellBar.appendChild(filled);
        if (widthVal < 100) {
          const emptySeg = document.createElement("div");
          emptySeg.className = "cell-segment";
          emptySeg.style.width = (100 - widthVal) + "%";
          emptySeg.style.backgroundColor = "#f5f5f5";
          cellBar.appendChild(emptySeg);
        }
        const overlay = document.createElement("span");
        ...
        overlay.style.fontSize = "0.6rem";
        overlay.style.fontWeight = "bold";
        overlay.style.color = widthVal > 50 ? "#ffffff" : "#000000";
        cellBar.appendChild(overlay);
        td.appendChild(cellBar);
        tr.appendChild(td);
      });
      nameTd.addEventListener("click", () => {
        // Remove existing details row if toggling off
        const next = tr.nextSibling;
        if (next && next.classList && next.classList.contains("detail-row")) {
          tr.parentNode.removeChild(next);
          return;
        }
        // Determine active projects for this consultant across selected months
        const activeProjects = new Set();
        monthPeriods.forEach((period) => {
          const allocs = (data.periods.month[period] && data.periods.month[period][consultant]) || {};
          Object.keys(allocs).forEach((proj) => {
            if (allocs[proj] > 0) activeProjects.add(proj);
          });
        });
        const projList = data.projects.filter((p) => activeProjects.has(p));
        // Build details row with a table showing allocations per month and project
        const detailTr = document.createElement("tr");
        detailTr.className = "detail-row";
        const detailTd = document.createElement("td");
        detailTd.colSpan = monthPeriods.length + 1;
        const detailsDiv = document.createElement("div");
        detailsDiv.className = "details";
        const detTable = document.createElement("table");
        detTable.className = "details-table";
        // Header row: Project + month labels
        const thead = document.createElement("thead");
        const headerRow = document.createElement("tr");
        const projTh = document.createElement("th");
        projTh.textContent = "Project";
        headerRow.appendChild(projTh);
        monthPeriods.forEach((period) => {
          const th = document.createElement("th");
          const { start } = getMonthStartEnd(period);
          th.textContent = start.toLocaleDateString(undefined, {
