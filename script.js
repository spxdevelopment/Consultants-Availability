...
(() => {
   ...
   */
  function updatePeriodInfo(viewMode, period) {
    const infoEl = document.getElementById("periodInfo");
    if (!infoEl || !period) return;
    let label = "";
    let range = "";
    if (viewMode === "week") {
      const { start, end } = getWeekStartEnd(period);
      // Display as "YYYY Week ##".  We additionally compute the
      // Friday date (end of work week) for clarity.  For example,
      // 2025‑W44 becomes "2025 Week 44 (ends Oct 31, 2025)".  The
      // full ISO week still spans Monday–Sunday, but showing the
      // Friday end date aligns with corporate calendars where the
      // work week concludes on Friday.
      const weekLabel = period.replace("-W", " Week ");
      label = weekLabel;
      const friday = new Date(start);
      friday.setDate(start.getDate() + 4);
      range = `ends ${formatDate(friday)}`;
    } else {
      const { start, end } = getMonthStartEnd(period);
      label = period;
      range = `${formatDate(start)} – ${formatDate(end)}`;
    }
    infoEl.textContent = `${label}: ${range}`;
  }
  ...
  function exportSummaryToCSV(data) {
    ...
    // Use a filename containing the period for clarity
    link.download = `allocation_${viewMode}_${period}.csv`;
  ...
  function renderSummary(data) {
    ...
    summaryEl.innerHTML = "";
    if (!period) return;
    // Update period information banner with date range
    updatePeriodInfo(viewMode, period);

    // Also render the weekly grid when in week view
  ...
  function renderWeeklyGrid(data) {
    ...
    container.innerHTML = "";
    const year = document.getElementById("yearSelect").value;
    // Collect week periods for the selected year
    const weekPeriods = Object.keys(data.periods.week)
