...
(() => {
  ...
  function renderTable(data) {
    ...
    data.projects.forEach((project) => {
      ...
      // Rename button for updating the project name.  Labelled as
      // "Rename" to clarify its purpose.  When clicked, prompt for a new
      // name and update the dataset if valid.
      const renameBtn = document.createElement("button");
      renameBtn.textContent = "Rename";
      renameBtn.className = "save-button";
      renameBtn.style.marginRight = "0.5rem";
      renameBtn.addEventListener("click", () => {
        const newName = prompt("Enter new project name:", project);
        if (!newName || newName.trim() === "" || newName === project) return;
        if (data.projects.includes(newName)) {
          alert("A project with that name already exists.");
          return;
        }
        renameProject(data, project, newName.trim());
        renderTable(data);
      });
  ...
  // Rename an existing project.  Updates the project list, metadata and all
  // allocation entries across periods.  Saves the dataset after changes.
  function renameProject(data, oldName, newName) {
    // Update projects array
    data.projects = data.projects.map((p) => (p === oldName ? newName : p));
    // Update projectsInfo metadata
    if (data.projectsInfo && data.projectsInfo[oldName]) {
      data.projectsInfo[newName] = data.projectsInfo[oldName];
      delete data.projectsInfo[oldName];
    }
    // Rename allocations in week and month periods
    ["week", "month"].forEach((mode) => {
      Object.keys(data.periods[mode]).forEach((period) => {
        const periodData = data.periods[mode][period];
        Object.keys(periodData).forEach((consultant) => {
          const alloc = periodData[consultant];
          if (alloc && alloc.hasOwnProperty(oldName)) {
            alloc[newName] = alloc[oldName];
            delete alloc[oldName];
          }
        });
      });
    });
