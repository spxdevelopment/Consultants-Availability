...
(() => {
  ...
  // Colour palette for projects used in the editing grid.  These
  // correspond to the assignments defined in script.js.  If new
  // projects are added, add a corresponding entry here so that cells
  // display in the appropriate colour.  Vacation is not shown in the
  // editing page colours as it is handled separately in the overview.
  const projectsColors = {
    "Stand Together": "#4CA09A",
    "EisnerAmper": "#147E76",
    "OPOS, Inc.": "#ee6c3c",
    "Omega Healthcare Management Services": "#CA2027",
    "iMethods": "#f9844a",
    "Divurgent, LLC": "#ffb703",
    "EQUIPX": "#219ebc",
    "OmniSource": "#90be6d",
    "SPX Outreach": "#577590",
    "SPX Sales": "#ffafcc",
    "SPX Management / Operations": "#8e7dbe",
    "Vacation": "#F9C74F",
  };
  ...
  function renderEditTable(data, consultant) {
    ...
    periods.forEach((period) => {
      ...
      if (viewMode === "week") {
        // Display the Friday of the ISO week period for clarity
        ...
        monday.setDate(mondayOfWeek1.getDate() + (weekNo - 1) * 7);
        const friday = new Date(monday);
        friday.setDate(monday.getDate() + 4);
        labelTd.textContent = friday.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });
      } else {
        // Month view: display as 'MMM YYYY'
        const [yr, mo] = period.split("-");
        const monthIndex = parseInt(mo, 10) - 1;
        const dt = new Date(parseInt(yr, 10), monthIndex, 1);
        labelTd.textContent = dt.toLocaleDateString(undefined, { month: "short", year: "numeric" });
      }
  ...
  function init() {
    ...
    // Sidebar in edit page shows navigation; consultants list is not populated here.
    // Populate year options
    ...
    // Back button navigates to consultants page
